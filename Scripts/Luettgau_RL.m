%Simple reinforcement learner (Rescorla-Wagner update) with learning
%rate(s) (alpha) and softmax temperature (tau) as free paramters
...
nsteps = 30; %30 define steps within the parameter space/numer of parameter constellations to be tested for initial grid search (starting point)
           ...
           if inivals == 2 %only perform grid search if grid search informed in inital values are needed
            ...
            for aa = 1:numel(space_alpha1)
                for bb=1:numel(space_alpha2)
                    grid(aa,bb) =  rl_candyman(version,mechanism, u_sub_r, r, rev_s, r_rev, choices, choice_trials, forcedchoices, [space_alpha1(aa),space_tau(bb)]); %write returned energy parameter of the RL function to grid of alpha/tau  
                end
            end
...
%                 disp(aa); drawnow
                for bb=1:numel(space_alpha2)
                    for cc=1:numel(space_tau)
                        grid(aa,bb,cc) =  rl_candyman(version,mechanism, u_sub_r, r, rev_s, r_rev, choices, choice_trials, forcedchoices, [space_alpha1(aa), space_alpha2(bb), space_tau(cc)]); %write returned energy parameter of the RL function to grid of alpha/tau  
                    end
                end
            end
...
%                 disp(aa); drawnow
                for bb=1:numel(space_alpha2)
                    for cc=1:numel(space_alpha3)  
                        for dd=1:numel(space_tau)
                            grid(aa,bb,cc,dd) =  rl_candyman(version,mechanism, u_sub_r, r, rev_s, r_rev, choices, choice_trials, forcedchoices, [space_alpha1(aa), space_alpha2(bb), space_alpha3(cc), space_tau(dd)]); %write returned energy parameter of the RL function to grid of alpha/tau  
                        end
                    end
                end
            end
            ...
            if alphas == 1            
               fun = @(x) rl_candyman(version,mechanism, u_sub_r, r, rev_s, r_rev, choices, choice_trials, forcedchoices,[x(1) x(2)]);
            elseif alphas == 2
               fun = @(x) rl_candyman(version,mechanism, u_sub_r, r, rev_s, r_rev, choices, choice_trials, forcedchoices,[x(1) x(2) x(3)]);
            elseif alphas == 3
               fun = @(x) rl_candyman(version,mechanism, u_sub_r, r, rev_s, r_rev, choices, choice_trials, forcedchoices,[x(1) x(2) x(3) x(4)]);
            end
